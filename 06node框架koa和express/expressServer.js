...
app.use((req,res,next) => {
    ...
    next()
    /**
     * 关于为什么是在 res 上设置header
     *
     * 跨域 并非 浏览器限制了发起跨站请求，而是跨站请求可以正常发起，
     * 但是返回结果被浏览器拦截了。 每次需求都会发出，
     * 服务器端也会做出响应，只是浏览器端在接受响应的时候会基于同源策略进行拦截。
     *
     * 在处理跨域请求时，浏览器会遵循同源策略，即浏览器只允许请求与当前页面的域名同源的资源。
     * 如果请求的域名与当前页面的域名不同，浏览器会阻止这次请求。但是，在实际开发中，
     * 我们可能会需要请求不同域名的资源，这时候就需要进行跨域请求。为了允许跨域请求，
     * 服务器需要在响应中设置一些跨域相关的响应头。例如，
     * 在响应中设置`Access-Control-Allow-Origin`响应头，可以允许指定的域名进行跨域请求。
     * 如果服务器没有设置这个响应头，浏览器会阻止跨域请求。
     * 所以，需要在响应中设置跨域相关的响应头，以允许跨域请求。
     */
})
app.use((req,res,next) => {
    console.log(2)
